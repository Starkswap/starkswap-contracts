...
// @author Mesh Finance
// @license MIT
// @notice Upgradeable proxy for StarkswapV1Router.cairo
...
func constructor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    implementation_hash: felt, factory_address: felt, pair_proxy_class_hash: felt, pair_class_hash: felt, proxy_admin: felt
) {

    Proxy._set_implementation_hash(implementation_hash);

    let calldata: felt* = alloc();
    assert [calldata] = factory_address;
    assert [calldata + 1] = pair_proxy_class_hash;
    assert [calldata + 2] = pair_class_hash;
    assert [calldata + 3] = proxy_admin;
    library_call(
