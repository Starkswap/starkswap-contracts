...
from starkware.cairo.common.uint256 import (
    Uint256, uint256_add, uint256_le, uint256_eq, uint256_sub, uint256_signed_div_rem
)
...
func _pair_for{syscall_ptr: felt*,  pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (pair_address: felt) = get_contract_address{hash_ptr=pedersen_ptr}(salt=0, class_hash=pair_class_hash, constructor_calldata_size=3,
        constructor_calldata=cast(new (base_address, quote_address, curve), felt*), deployer_address=factory_address)
...
func _get_reserves{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    token_a_address: felt,
    token_b_address: felt,
    curve: felt) -> (reserve_a: Uint256, reserve_b: Uint256):

    alloc_locals

    let (factory_address: felt) = sv_factory.read()
    let (local pair_address: felt) = IStarkswapV1Factory.getPair(factory_address, token_a_address, token_b_address, curve)
    with_attr error_message("StarkswapV1Router: INVALID_PATH"):
        assert_not_zero(pair_address)
    end
    let (reserve_0: Uint256, reserve_1: Uint256, block_timestamp_last: felt) = IStarkswapV1Pair.getReserves(contract_address=pair_address)
    let (base_address: felt, quote_address: felt) = _sort_tokens(token_a_address, token_b_address)
...
func _get_or_create_pair{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (factory_address: felt) = sv_factory.read()
    let (existing_pair_address: felt) = IStarkswapV1Factory.getPair(factory_address, token_a_address, token_b_address, curve)
    if existing_pair_address == FALSE:
        let (new_pair_address: felt) = IStarkswapV1Factory.createPair(factory_address, token_a_address, token_b_address, curve)
...
func _add_liquidity{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (reserve_a: Uint256, reserve_b: Uint256) = _get_reserves(token_a_address, token_b_address, curve)
...
func addLiquidity{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (amount_a: Uint256, amount_b: Uint256, pair_address: felt) = _add_liquidity(token_a_address, token_b_address, curve,
        amount_a_desired, amount_b_desired, amount_a_min, amount_b_min)

    let (caller_address : felt) = get_caller_address()

    IERC20.transferFrom(contract_address=token_a_address, sender=caller_address, recipient=pair_address, amount=amount_a)
    IERC20.transferFrom(contract_address=token_b_address, sender=caller_address, recipient=pair_address, amount=amount_b)
...
func removeLiquidity{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    IStarkswapV1Pair.transferFrom(contract_address=pair_address, sender=caller_address, recipient=pair_address, amount=liquidity)
    let (amount_0: Uint256, amount_1: Uint256) = IStarkswapV1Pair.burn(contract_address=pair_address, to=to)
    let (base_address: felt, quote_address: felt) = _sort_tokens(token_a_address, token_b_address)

    let (local amount_a: Uint256, local amount_b: Uint256) = _sort_amounts(token_a_address, base_address, amount_0, amount_1)
...
func _swap{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (pair_address: felt) = _pair_for(route.input, route.output, route.curve)
    IStarkswapV1Pair.swap(contract_address=pair_address, base_out=base_out, quote_out=quote_out, to=to_address, calldata_len=0, calldata=cast(new (), felt*))
...
func swapExactTokensForTokens{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    with_attr error_message("StarkswapV1Router: INSUFFICIENT_OUTPUT_AMOUNT"):
        let (amount_out_min_le_return_amount) = uint256_le(amount_out_min, amounts[amounts_len - 1])
    ...
    let (pair_address: felt) = _pair_for(route.input, route.output, route.curve)
    let (caller_address : felt) = get_caller_address()
    IERC20.transferFrom(contract_address=route.input, sender=caller_address, recipient=pair_address, amount=[amounts])
